...
| Path | What it does |
|---|---|
| `/admin/system/health` · `operational-health` | Subsystem health, timings, stale data, backlogs, Redis health, plus **Upstream Data Sources** (2026-08-29): is ESPN / Open-Meteo / NWS / the Odds API actually answering us right now, and how long any of them has been failing. |
...
| `/admin/system/seo-health` | Read-only SEO architecture snapshot (sitemaps, hubs, ZIP tiers, canonical host), plus **live Search Console numbers** since 2026-09-08 — impressions, clicks, average position, top pages and top queries over a 28-day settled window, pulled weekly by `/api/cron/gsc-monitor`. The CSV import panel still works and is the fallback when no service-account credential is configured; the panel tells you which state you are in. |
...
**One carve-out on that rule, added 2026-08-27 per Derek and widened 2026-08-31:** the **Weatherboards**
keep showing a market after it locks, greyed out and labeled `closed`, **and it is now linked**. The three hours before kickoff plus game time are peak interest, and a
blank cell there reads as "there is no market for this game" rather than "this
one has closed." Per Derek (2026-08-31): "even if a wager is closed on /weatherboard or /weatherboard/extended, it should still be hyperlinked." So a **locked** market's page now opens read-only, and `/wagers/game` lists it. **The 2026-08-26 boundary is intact** — that rule was about the SETTLED book, and **graded and void are still admin-only and still 404**, as is `/api/wagers`, which lists nothing closed. What opens is a market awaiting its result, whose existence, terms and line the board was already showing on the same screen the customer clicked from; the page adds no information, it just stops the link being a dead end. **Betting is not gated by this and never was:** `placeBet` rejects any wager that is not open AND still before its lock time, server-side, so a viewable closed market cannot be bet on whatever a page renders. Two predicates now, and the names are the difference: `isPubliclyVisible` = may it be browsed and bet on, `isPubliclyViewable` = may it be shown read-only. A closed market's page is `noindex`, since it is unbettable and about to be graded and 404 again. Graded and voided markets still never appear on the board at all.

**A ZIP page shows the Wager on Weather consensus or it shows nothing (2026-08-29, per Derek: "zip pages should show wager on weather / consensus forecast")** — three states, and a customer can always tell which one they are looking at. Normally: the live consensus. When the upstream forecast is down but we hold a recent real one for that location (kept up to 6 hours): **the last real consensus**, behind an amber banner naming its age, because a two-hour-old forecast is still a true statement about the weather. When we hold nothing real: the forecast body is **replaced by a notice** and the page returns **HTTP 503 with `Retry-After`**, which is the correct answer for temporarily missing content and tells a crawler to come back rather than to record the empty state. What it never does is render the simulated fallback, which it did until this date.

**Every upstream reports its own silence (2026-08-29)** — new `data-source-health.ts` tracks ESPN (and, separately, ESPN's canonical host, which is the early warning), Open-Meteo, NWS and The Odds API. Three consecutive failures raises an alert naming the source AND what it costs the customer; a success clears the streak. All five are wired to their real fetch paths as of 2026-08-30, so a row reading "NO DATA YET" means we genuinely have not called that service since the record expired, not that the tracking is missing. NWS deliberately does not count a **404** as a failure: it answers that for the four venues outside its US coverage, and paging on those would make the alarm noise. Read it on `/admin/system/health` under **Upstream Data Sources**. **Why it exists:** on this date, two upstreams were dark for hours and the site looked completely normal, because every one of them has a fallback and a working fallback is exactly what makes an outage invisible. Logs are not an alarm. If you are adding a new upstream with a fallback, add it here in the same change.

**Nothing is priced on simulated weather (2026-08-29)** — when Open-Meteo cannot be reached (it rate-limits us), the app falls back to `getMockForecast`, a **generated** forecast: a seeded pseudo-random curve, not observed or modeled weather. That fallback is years old and had always been returned stamped `source: 'open-meteo'`, so nothing downstream could tell it from the real thing, and the auto-market engines priced real money against it. Three things changed. A generated forecast now carries **`synthetic: true`** on the response and a source label that says "Simulated (Open-Meteo unavailable)". It is **never cached**, so one rate-limited request no longer pins invented numbers into the shared cache for ten minutes of every reader. And every money path refuses it: the engines drop that venue and skip the game (raising a **critical alert**, type `forecast_simulated`), the plausibility audit refuses to corroborate against it, and the operator "Suggest Lines" helper returns no suggestion rather than a made-up one. **Why the audit needed its own guard:** it checks a line against that venue's own hourly forecast, which on a simulated response is the same invented series — a generator is internally consistent by construction, so the check would pass every time and mean nothing. If you see games being skipped with that alert, we are being rate-limited; that is the system working, not a new bug.
...
- **Results archive** (`/weatherboard/history`, added 2026-09-08): a day-by-day
  ...
  labeled as such on the board, never presented as a live quote. **The WES on a
  started or finished game is the score frozen when the game began** (since
  2026-09-24) and is labeled for the same reason, in the league's own word for
  that moment: **First pitch** on baseball, **Kickoff** on football and soccer
  (see `gameStartNoun` and `game-wes-snapshot.ts`). The records are written hourly by
  `/api/cron/archive-games` while the games are still current; nothing can
  reconstruct them afterward, because the schedule window starts at the current
  game day and The Odds API drops a game at kickoff. The archive therefore only
  reaches back to 2026-09-08.
...
- **2026-09-24**: **A game's WES is now frozen when the game starts, so a played game still has one.** Derek: "WES numbers are not showing up for every game. WES needs to always be there, even after the game has started and finished." On that evening's board, **15 of 17 rows were finals and only 4 rows anywhere had a WES chip**: the two games still in progress, and two that had just ended. **The bug was not in `wes.ts`.** `computeGameWes` scores the 3.5h window starting at first pitch by sampling the hourly forecast every 30 minutes, but `getForecast()`'s hourly array is trimmed to **"current hour onward"** everywhere in the app (see `open-meteo.ts`, and the comment in `league-schedule.ts` that already noted the same trap for kickoff snapshots). So a game's own window **drains out of the data as the game is played**: slot by slot while it is under way, and entirely once it ends. `getGameWindowForecast` then returns nothing, `computeGameWes` declines, and the chip disappears. The score was never wrong; there was nothing left to compute it from. **It cannot be recovered after the fact.** WES scores feels-like, gust, dew point, cloud, visibility, UV and precipitation rate together, and the post-game observation path (`game-forecast-accuracy.ts`) carries only temperature, wind and precipitation, so a WES rebuilt from the fields that happen to survive would not be the same number. **So it is frozen instead**, on exactly the model `game-closing-odds.ts` established for the sportsbook line in the next column: while the game is `pre` every full board render overwrites the stored copy, so what we hold converges on the last forecast before first pitch; from kickoff onward we stop writing and the frozen copy is what the board shows, **outright** rather than only when the live value is missing. **Freezing is the more honest number, not just the available one.** WES scores an EVENT (what this weather does to the experience of attending and playing it), not an instant. Recomputing mid-game over whatever is left of the window meant the chip in the 9th inning described the last half hour rather than the game, and drifted downward under a reader all evening. A game gets one WES: the one its full window was forecast to have. The board labels it the way the odds column labels a **Closing line**, and the red **Live** tag now appears only in the rare case the score really is a live computation. **The label is the league's own word for the start of a game**, per Derek the same day: "you've got 'first pitch' as a football term. 'first pitch' is baseball, football and soccer are 'kick offs'." So it reads **First pitch** on baseball and **Kickoff** on football and soccer, from the new `gameStartNoun` in `league-schedule.ts`, which reads the venue's own `sport` (reliable for every venue a board can reach, since the only `multi` venues in venue-data are community fields no league schedule touches). The game-weather narratives in `game-weather-narrative.ts` already split this way per league; this brings the short labels around them in line. The flag behind it is called `wesIsFrozen` rather than anything sport-specific for the same reason. **Coverage does not depend on anyone loading the page:** `/api/cron/archive-games` runs the full enrichment path for all four leagues hourly at `:25`, so every game is enriched as `pre` several times before it starts. And because the archive stores the enriched game whole, finished games now carry their WES into `/weatherboard/history` permanently. **Games that had already finished before this shipped stay blank**, because nothing was ever frozen for them and, per the paragraph above, nothing can be. **Still shows no WES by design:** an indoor or roof-closed game, where the board says weather is not a factor at all. New `src/lib/game-wes-snapshot.ts`, new `wesAtFirstPitch` on `EnrichedScheduleGame`, both Weatherboard tables updated; tests in `tests/game-wes-snapshot.test.ts` pin the freeze boundary (including that a half-elapsed window's live score **loses** to the frozen one), that a started game with nothing frozen is never *labeled* first-pitch, and that the venue-plus-kickoff-hour key survives a game changing feeds.

- **2026-09-12**: **Retractable roofs: we now read the roof state instead of assuming it is open.** Derek: "are you sure the toronto mlb game is outdoors?" and then "you've got to figure out the roof status of these venues." **We were not sure.** Roof state came from `getRoofStatus(gamePk)`, a per-game MLB live-feed call reading `gameData.weather.condition`, and that block is **empty until the game reaches Pre-Game**. The code turned that empty answer into *roof open*, so the board described wind direction, gusts, sun glare and a WES of 99 for Rogers Centre when the roof may well have been shut. **What the API actually provides, verified live on 2026-09-12:** the schedule endpoint's **`weather` hydrate** carries the same condition line, and it is populated from **Pre-Game onward and stays after Final**. Rogers Centre read "Sunny, 6 mph R To L" (open) while loanDepot park read "Roof Closed, 0 mph None" on the same slate; the previous day both Chase Field and loanDepot were "Roof Closed" and American Family Field was "Clear, In From CF". Only a still-**Scheduled** game returns `{}`. **So the roof now rides in on the schedule fetch we already make** (`hydrate=probablePitcher,linescore,weather`), which **removes a per-game live-feed request for every retractable game on the slate** (each pulled a full play-by-play document to read one string) and **widens the window**: the old check only ran for TODAY, but the feed reports from Pre-Game and keeps it after Final, so a finished board can state what the roof actually did and today's knows hours before first pitch. **Unknown is now its own answer.** New `roofStatus` ('open' | 'closed' | 'unknown') and `roofUncertain` on the schedule; when a retractable venue has no admin override, no season-long call and no reported condition, the weather line is prefixed **"Roof status not yet reported, conditions below assume it is open"** rather than silently asserting open air. The forecast is still shown, it is just no longer stated as fact. **Scope note:** this was always display-only for Toronto, since `mlb-tor` is in `NON_US_VENUE_IDS` and no auto-market is ever created or graded at Rogers Centre. Tests in `tests/roof-status.test.ts` pin that an empty condition is unknown and never open, that "Roof Closed" wins in any casing, that a real sky description means open, and that the caveat appears without swallowing the forecast.

- **2026-09-12**: **MLS and NWSL markets were showing up on MLB game rows, and the boards were not in rotation order.** Two Weatherboard bugs from Derek. **(1) Wrong markets on a row.** Per Derek: "you've got two wagers on team 926", then 917, then the Reds. The Washington Nationals row carried an **Audi Field** market, the Houston Astros row carried two **Shell Energy Stadium** markets (Dynamo and Dash), and the Reds row carried a **TQL Stadium** one. `locationMatchesVenue` decided which markets belong to a game by COORDINATES ALONE, within `VENUE_COORDINATE_TOLERANCE_DEG` (0.05 degrees, about 3.5 miles). That tolerance exists to absorb city-centroid slop for a market stored with only a city label, but a city puts its stadiums far closer together than that: **Audi Field is 0.005 degrees from Nationals Park and Shell Energy Stadium 0.005 from Daikin Park, ten times inside the tolerance**, so every such pair matched every time. This is the same family as the 2026-09-08 Lumen/T-Mobile fix, which corrected the DISPLAY name by exactly this rule and left the matching alone. Now a stored label that is already a tracked venue decides by **identity**, and the coordinate path remains only for the case it was written for, a location stored as a bare city or ZIP. `/wagers/game` carried its own copy of the matcher, commented "matches weatherboard-markets.ts", which is precisely how it kept the bug after the board was fixed; it imports the shared one now. Tests pin the DC and Houston pairs and the city-label fallback. **(2) Rotation order.** Per Derek: "none of the schedules are in rotation number order for any sport". The boards print the rotation number in their first column but listed games by kickoff time, so that column jumped around. Games now sort by ET day first, then rotation number, then kickoff. Day first is deliberate: rotation numbers restart each day, so sorting on them alone would interleave tomorrow's early game with today's late one and break the date grouping the table renders from. A game whose rotation is not known yet sorts after the numbered games for its day rather than to the top, because an absent number is unknown, not zero.

- **2026-09-11**: **The homepage's open-market cards were missing the temperature.** Per Derek: "on open weather markets on the main page of wageronweather.com, you don't have the temperatures." A card for "Donald W. Reynolds Razorback Stadium Temp at Game Start" showed two chips reading **Over -110** and **Under -110** and never printed **81.5°F**, so the card asked a question without stating the number it turns on. Pointspread cards were the same, showing each stadium and -110 but never the **-47.5°F** spread. **The fix already existed and this one component had never adopted it:** `outcomeTarget` in `public-wager-display.ts` was extracted on **2026-08-24** for precisely this bug, when `/wagers` cards and the market detail page had it, and `FeaturedMarkets.tsx` was left behind. The data was never missing (`line`, `spread` and `unit` are all on the public API payload), only unrendered. Swept the other public surfaces at the same time: `WagerCard` and `WagerDetailPage` already use the helper, and nothing else renders outcome tiles, so the homepage was the last one.

- **2026-09-11**: **Forecast Performance: the Forecast Tracker's history is now readable as graphs, at `/admin/forecast-performance`.** Per Derek. The tracker has been logging every source's forecast against the NWS observation that settled it since 2026-03, **5,524 entries, 5,290 of them scored**, and nothing read them back except one flat table. The new page is shaped like Search Console: four clickable metric tiles (mean absolute error, accuracy score, bias, forecast count) over one time series, a filter row, and dimension tabs (**Source / Measure / Location / Lead time**) with a sortable table underneath. Each tile shows the change against the previous equal-length period. **Two deliberate departures from Search Console.** (1) **One metric on the chart at a time.** Search Console stacks clicks, impressions, CTR and position on hidden separate axes, which invents correlations between series whose scales are unrelated; here the tiles SELECT the charted metric and the lines are the SOURCES, which share one scale and are the actual comparison. (2) **The units are policed.** Temperature error is °F and wind error is mph; averaging them is a number with no meaning, so a mixed selection refuses to print a unit and says to filter to one measure or use the accuracy score, which is comparable across both. **What the data already says** (temperature, like-for-like at the 3-5 day lead where most of the book sits): **Wager on Weather 2.57 mean absolute error, NWS 2.77, Weather.com 4.23, AccuWeather 4.56, raw Open-Meteo 4.13.** Our consensus is the most accurate source on the tracked sample, and it beats the raw Open-Meteo model it is built from by a wide margin, which is the clearest evidence yet that the NWS-weighted blend is doing real work. **Read the source table with care:** each source is logged at its own mix of lead times (the consensus is 92% at 3-5d, AccuWeather only 19%), and a ten-day forecast is a harder forecast, so the page says so and points at the Lead time filter. Reachable from the Forecasts and Wagers tab rows, and from the tool directory. Aggregation lives in `forecast-analytics.ts` as pure functions with 12 tests, including that an unverified forecast never averages in as a zero and that a source with no data in a bucket draws a GAP rather than a line plunging to the floor.

- **2026-09-11**: **MLB can now see a neutral site at all, and the boards say when a game is not at either team's park.** Follow-up sweep of all four leagues, per Derek. **MLB was resolving a game's venue purely from the HOME TEAM'S NAME** and never reading the venue the MLB Stats API reports, so London, Mexico City, Tokyo, Field of Dreams and the Little League Classic would every one of them have been priced and graded at that team's regular park, exactly the football bug. It captures `feedVenueName` now and treats a disagreement with the home park as a neutral site. **There is no live MLB defect today:** of 33 venues across the rest of the season the only untracked names are the postseason placeholders "AL Stadium", "NL Stadium" and "TBD". This closes it before next June. **MLS/NWSL: 210 games scanned, zero neutral-site, nothing to do.** **NFL: 9 neutral-site games this season and every single one is an untracked international venue** (Melbourne, Maracanã, Tottenham x2, Wembley, Stade de France, Bernabéu, Munich, Estadio Banorte), so the drop-the-game regression would have hidden nine games, not one. **Both Weatherboards now print "Neutral site: <venue>" under the home team**, and add "(weather shown is <park>)" when we don't track the real ground and the row is showing the home park's forecast as a stand-in. That last part matters: the 09-10 Melbourne game rendered as "Indoors" with SoFi Stadium's conditions, which is true of SoFi and false of the game.

- **2026-09-11**: **A neutral-site game must not disappear from the board, and the cross-venue markets pair the two TEAMS' parks.** Two corrections to the 09-10 neutral-site fix, both from Derek. **(1) The game vanished.** Per Derek: "tonight's NFL game didn't show up on the weatherboards." The 09-10 NFL game, 49ers at Rams, was played at **Melbourne Cricket Ground** (`neutralSite: true`). Resolving the true venue is correct, but `league-schedule.ts` then did `if (!venue) continue` when that venue is not one of our 273 tracked ones, so the fixture was dropped from every Weatherboard instead of merely wearing the wrong park's name. **That is strictly worse than the bug it replaced:** invisible beats mislabelled only if you never wanted the game at all. A market-correctness rule had been applied to the whole schedule, display surfaces included. An untracked neutral site now falls back to the home team's park so the fixture stays visible, and carries **`venueIsGameSite: false`** plus **`gameSiteName`** ("Melbourne Cricket Ground") so the truth travels with it. The venue over-under engine refuses any game with `venueIsGameSite` false, since "Temp at Game Start" priced on a stand-in park is exactly the original defect. **(2) The pairing was wrong.** The cross-venue pointspreads had been pairing the true game venue against the AWAY team's park, which quietly dropped the home team's own stadium out of its own fixture (Bank of America vs Milan Puskar, with Scott Stadium nowhere). They now pair the two teams' parks, via a new **`homeTeamVenue`** field that equals `venue` for every ordinary game and differs only at a neutral site. The pointer key still uses `venue`, so one fixture is still one market set. `buildVenueForecasts` fetches the home team's park too, since at a neutral site it is a third venue and the cross-venue markets would otherwise skip with "forecast fetch failed". Tests pin the Melbourne case and the pairing.

- **2026-09-10**: **Neutral-site games were priced at the wrong stadium, and every one of them was a duplicate waiting to happen.** Five identical pairs of open markets, ten in all, existed for Virginia vs West Virginia on 09-19. The 2026-09-08 pointer fix (venue plus kickoff hour) was working correctly; this was a different cause sitting upstream of it. **That game is played at Bank of America Stadium in Charlotte, at neither team's stadium.** `league-schedule.ts` resolved a game's venue as *the home team's* stadium and never looked at ESPN's `competition.venue`, so the fixture resolved to Scott Stadium. The Odds API, which names the OTHER team as home for these games, resolved it to Milan Puskar Stadium. Two venues for one fixture means two different pointer keys, so both engines created a full set of markets, three days apart. The schedule merge could not catch it either: its name check compares an ORDERED pair ('virginia|westvirginia' against 'westvirginia|virginia') and its venue check compares Scott against Milan Puskar, so both missed and the same game was added to the slate twice. **The duplication was the visible half; the wrong stadium was the worse half.** A market titled 'Temp at Game Start' was grading the temperature at a stadium hundreds of miles from the field, and unlike the 09-08 labelling bug this one *did* reach settlement, because the venue is what the market is priced and graded on. Both halves are fixed: a neutral-site game now resolves to the stadium ESPN says it is played at (the same rule `espn-football-schedule.ts` has always used for the public CFB report, so the report and the markets finally agree), and the merge treats the same two teams within 90 minutes as one fixture whichever way up the feeds list them. The team-name check stays time-bounded so a home-and-home, whose legs sit days apart, is still correctly two games. `venue.team` is no longer used as the home team's name at a neutral site, where it is the resident pro tenant: a college bowl at Bank of America Stadium would have listed **Carolina Panthers** as the home team. **Scale: this is a bowl-season bug.** Only one game in the current window is affected, but ESPN lists **47 neutral-site college games** across week 1 and bowl season, 34 of them at venues we track; the other 13 (Aviva Stadium, the Sun Bowl, Camping World Stadium) now create no market rather than a market at the wrong stadium. All ten Virginia/West Virginia markets were voided, none of which had taken a bet, and the corrected engine reminted them against Bank of America Stadium. **Spotting a recurrence:** two markets for one fixture whose titles name two different stadiums, both of them the teams' own. Tests in `tests/odds-schedule-fallback.test.ts` pin the home/away flip and the home-and-home guard.

- **2026-09-08**: **Markets were displaying the wrong stadium, and duplicate markets were being created for one game.** Two bugs from one Weatherboard question (Derek: "why are there two bets so similar?"). **(1) Duplicates.** The auto-market engines key a Redis pointer on the feed's game id to remember they already built a market for a game, and that id is not stable: ESPN supplies a numeric event id, the Odds API fallback supplies `odds-<pair>-<ms>` for the same game. **ESPN's canonical host began 403ing our egress on 2026-08-29**, so every game came back under a fallback id, the pointer missed, and a second market was minted at whatever the forecast said that day. Every duplicate on the 2026-09-09 slate was one copy from before the 29th and one after, which is also why their lines disagreed (Gillette/Seattle High vs High sat at both +2.5 and -3.5). Pointers now use venue plus kickoff hour, the same identity `rotationKey` and the schedule merge already use, and `getMappedWagerId` migrates the old id-keyed pointer forward so the first run after deploy did not mint a THIRD copy. The exact hour bucket is deliberate: a wider window would let a same-venue doubleheader collide and leave the second game with no market, which is worse than a duplicate. **Spotting a recurrence: the orphaned copy is the one that stopped updating** — on the 09-09 slate every duplicate created 08-25 froze on 08-30, while its twin was still repricing on 09-08. **(2) The wrong stadium.** Derek: "Seattle Mariners play at T-mobile but the Seattle Seahawks play at Lumen field." An NFL market created against **Lumen Field** was shown, in its title AND its terms, as **T-Mobile Park**. The three display paths resolved a venue name by coordinate match, and `findVenueByCoords` returns the first venue within `VENUE_COORDINATE_TOLERANCE_DEG` (0.05 degrees, about 3.5 miles); Lumen Field sits roughly 1,300 feet from T-Mobile Park. **61 of 276 venues resolved to a different venue** this way, most of them an NFL stadium wearing its city's ballpark name: Soldier Field as Rate Field, Ford Field as Comerica Park, Acrisure Stadium as PNC Park. New `resolveVenueDisplayName` trusts a stored label that is already a tracked venue, since that is the venue the market was created against and grades on, and keeps the coordinate lookup for the case it was added for on 2026-08-24, a location stored as a bare city. **Grading was never affected** — it reads the stored location's own coordinates and NWS station, not the label — so this was a labelling bug on the board and market pages, not a settlement one. Tests in `tests/venue-display-name.test.ts` pin the Lumen/T-Mobile pair, the city upgrade, and a guard asserting no tracked venue displays as a different venue.

- **2026-09-08**: **The Forecast Tracker's Wager on Weather column was blank for today, for three metrics that had nothing wrong with them.** Per Derek: "when inputing forecasts in the admin, it isn't pulling anything for wager on weather, only for Art and NWS." The auto-pull withheld the **Wager on Weather** value for **any** metric whenever the target date was today. The reason it withheld it is sound and unchanged: `getForecast()` floors today's high and low with observations already recorded, so scoring that as a forecast flatters the result. But the floor's reach is much narrower than the guard was. `applyObservedExtremes()` rewrites `daily[0].highF` and `daily[0].lowF` and **nothing else** — not the hourly temperatures, not the daily wind fields — so today's **actual_temp**, **wind_speed** and **wind_gust** were clean forecasts being thrown away, and those are exactly the metrics an operator logs for a game being played today. The guard now names the two contaminated metrics instead of the date. **The second half of the bug was silence.** The endpoint has always explained itself — out of horizon, floored same-day high/low, provider error — but the tracker only displayed those warnings when **zero** sources filled. A partial pull reported "filled 2 source(s)" and dropped the reason for the third, which is why a deliberate, documented decision read as a broken feature. Warnings now show on partial fills too. The pull button also still read "Pull from Open-Meteo + NWS", copy that predates the consensus column being added in July and actively told operators the Wager on Weather row was not part of the pull; it now names all three. **AccuWeather and Weather.com remain manual by design** — neither has a feed we can automate — so two empty columns after a pull are still correct. A new case in `tests/forecast-observed-floor.test.ts` pins the floor's blast radius to `highF` and `lowF`, so extending the floor later cannot silently invalidate the guard.

- **2026-09-08**: **Search Console is measured on a schedule now, and `/admin/system/seo-health` shows live numbers.** Per Derek. That page had been fed by hand — an operator exported two CSVs from Search Console and uploaded them — which works exactly as often as somebody remembers, and over the previous eight weeks that was never. **What the first real measurement found, over the 30 days to 2026-09-08:** `/mlb-weather` 61 impressions, `/nfl-weather` 46, `/college-football-weather` 15 **and the only click on the site**; everything else 10, across roughly 6,300 ZIP pages. **Every single query was sports-weather** — "weather forecast for nfl games", "reds game weather", "rotowire mlb weather", "ncaa football weather" — and not one was the generic "[city] weather" the ZIP pages are built for. The niche pivot is working and the ZIP surface is not. Two CFB queries already rank inside the top 15 ("ncaaf weather today" at 10, "ncaa football weather" at 13). New `gsc-api.ts` reads the Search Analytics and URL Inspection APIs with a **read-only** (`webmasters.readonly`) service-account credential and **produces the same row shapes the CSV importer produces**, so the reconciliation layer, the recommendation queues and the dashboard are untouched and cannot tell where the rows came from; the CSV path still works and is the fallback when no credential is set. `gsc-monitor.ts` plus a weekly `/api/cron/gsc-monitor` (Mondays 12:00 UTC) snapshots a 28-day settled window, keeps a year of snapshots, and compares each to the last. **What it alerts on is the careful part:** impressions, not clicks (clicks are far too sparse here to carry signal), and only above a **floor of 40 baseline impressions**, because a site at this volume produces a "75% decline" going from 4 impressions to 1 and an alarm that fires every week gets muted — taking the real one with it. Same rule as `data-source-health.ts`: alert when a customer would notice. A failed pull is never stored, since an empty snapshot would become next week's baseline and manufacture first a fake recovery and then a fake collapse. **Setup:** add a Google service-account key as `GSC_CREDENTIALS_BASE64` (or reuse the existing BigQuery `GCP_CREDENTIALS_BASE64`) and add that account's email as a user on the Search Console property; the page tells you which state it is in. Tests in `tests/gsc-monitor.test.ts` pin the floor, the no-baseline case, the settled-window lag, and the coverage-state mapping.

- **2026-09-08**: **The Weatherboards are in the sitemap, and the blocked ESPN host is no longer tried first.** Two findings from the same session. **(1)** A URL inspection came back **"URL is unknown to Google"** for `/weatherboard` — not crawled, not indexed, no record of it at all — on the page the homepage's primary button now points at. It had never been in the sitemap and nothing linked to it from an indexed page, so there was no path in. The five board routes and the new results archive are in now. **(2)** ESPN's canonical host has 403'd our egress **continuously since 2026-08-29**, and trying it first on every fetch meant every scoreboard request paid a full failed round trip before the mirror answered — on every board render, every venue page, and four times over on the bare `/weatherboard`, which pulls four leagues. `/college-football-weather` measured **11.7 seconds** cold. `espn-scoreboard.ts` now remembers the host that answered and goes straight to it. **The 30-minute TTL is the re-probe and is deliberately not refreshed on success:** if every successful mirror fetch extended it, a busy site would never try the canonical host again and we would never learn it had recovered — trading a slow site for a permanently blind one. A canonical success clears the memo immediately. The `espn-primary-host` health row keeps moving twice an hour instead of going stale.
...
- **2026-08-31**: **Rotation numbers now survive the end of the game.** Per Derek: "don't erase the rotation numbers when the games end." The Weatherboards' `#` column reads `lines.homeRotation` / `awayRotation`, which arrive attached to a game in The Odds API's **/odds** response — an endpoint that only lists games a bookmaker is still taking action on. The moment a game started, it dropped out, `getGameLines` returned null, and the entire `lines` object went with it, including the two values in it that were never prices. **A rotation number is an identifier, not a market:** it is assigned before the game and does not change, and a final score keeps its rotation number on every printed sheet in the business. New `rotation-numbers.ts` remembers a game's pair for 7 days and fills it back in when the live feed no longer carries it. **A live value always wins**, so a corrected number still propagates rather than being pinned to the first one we ever saw. Keyed by **tracked venue plus kickoff hour**, not by game id, because the ESPN and Odds API ids differ for the same game and a game can switch between those sources between renders (same identity rule the duplicate-game fix uses). When only the remembered pair exists, the board gets a lines object carrying just the two numbers, and every price column renders `—` on its own, since `formatAmericanOdds` already answers that for a missing value. Tests in `tests/rotation-numbers.test.ts`.

- **2026-08-31**: **The same game was listed twice under two different names.** Derek, on the NCAA board for September 3: "you are duplicating." It was real — `Massachusetts Minutemen @ Rutgers` (ESPN, id `401858423`) sat directly above `UMass Minutemen @ Rutgers` (the Odds API fallback), and `UAlbany Great Danes @ Buffalo` above `Albany @ Buffalo`. **Caused by fixing ESPN earlier the same day:** while ESPN was 403ing there was only ONE source, so nothing could disagree; restoring it gave us two feeds that spell some schools differently, and `mergeOddsScheduleFallback` deduped on `homeTeam|awayTeam` alone. An alias list would have to grow forever. The merge now also matches on **the tracked home venue plus kickoff within 90 minutes**, which is what two feeds actually agree on: wide enough to absorb a feed disagreeing by minutes, narrow enough to leave a genuinely separate game alone. Tests cover the exact Rutgers case, a feed an hour out, and two different games at one venue hours apart. **Worth knowing for the next one:** a fix that restores a second source will surface every disagreement between the two that a single-source period was hiding. Also fixed in the same pass: `tests/cache-headers.test.ts` was **passing vacuously** — a `d` escape collapsed to a literal `d` between editor and disk, so every directive lookup returned 0 and every assertion compared 0 to a ceiling. It now parses by splitting on commas, and the first test pins the parser itself.

- **2026-08-31**: **The Weatherboards could be six minutes behind the game.** Chasing why the re-cut WES colors still looked old on Derek's screen: the page really was serving the old ones, from the edge. Every board set `Cache-Control: public, s-maxage=60, stale-while-revalidate=300` — 60 seconds of cache **plus five more minutes of deliberately serving the stale copy** while it revalidated behind the reader. Observed live: `X-Vercel-Cache: STALE`, `Age: 164`. The colors were only the visible symptom. **The same window applied to the live score, the period and the game clock**, which is what actually matters on a board someone reads during a game, and it is why a deploy can appear not to have landed. Now `s-maxage=30, stale-while-revalidate=30`, bounding the worst case at about a minute, in one shared constant (`src/lib/cache-headers.ts`) so the eleven board routes cannot drift apart. Deliberately not zero: an uncached render costs real upstream work, and a board a few seconds behind is honest where one six minutes behind is wrong. A test pins the ceiling at 60 seconds rather than the exact string. **Still on the old window and worth a decision:** the ZIP pages (`s-maxage=300, stale-while-revalidate=1800`, so up to **35 minutes**, on a page whose hero says "current conditions") and the venue pages (60/300). Both are far higher traffic, so tightening them trades cost for freshness.

- **2026-08-31**: **The top five WES bands were re-cut, because the scale was right and still unreadable.** Derek, looking at the MLB Weatherboard Extended: "the new color schemes for WES are not being applied." They were — the served HTML carried the exact specified hexes — but every game on that board scored 87 to 99, and as originally specified those bands ran `#118A5C` to `#064E5A`: five colors that get **darker and lose more than half their chroma** as conditions improve (C 0.145 at "Good" down to 0.066 at "Perfect"). Dark and desaturated is precisely where the eye discriminates worst, so the whole column read as one dark green and looked exactly like nothing had changed. **Worth internalising:** a color scale can pass every contrast and boundary check and still fail, because AA contrast is about text on a fill and says nothing about whether two fills differ from each other. The band the product shows most often is the one that has to separate best, and for a summer MLB slate that is 85+. The five now hold chroma (0.13 to 0.15) and **climb** in lightness, walking green to aqua (`#06A87C` Emerald, `#02BA89` Jade, `#19CDAC` Turquoise, `#44DFCC` Aquamarine, `#6EF0EB` Pale Aqua), so the best conditions are the brightest thing on the page rather than the most muted. An 87 and a 99 now sit **0.128 apart in OKLab where they were 0.090**, and all five read dark ink at better than 5:1 (up from 4.36:1 at worst). **The 0-79 half of the scale is untouched** — it was always legible. New tests in `tests/wes-scale.test.ts` pin the properties rather than the values: 87 vs 99 must stay perceptually apart, the top must get brighter rather than darker, and the top bands must keep their chroma.

- **2026-08-30**: **The source alarm now covers all five upstreams, and the rule is in `CLAUDE.md`.** Yesterday's alarm declared `nws` and `odds-api` but only wired ESPN and Open-Meteo, so two rows on the health page could never have turned red. Both are now reporting from their real fetch paths (all three Odds API endpoints, and both NWS forecast fetches). **NWS deliberately does not count a 404**, which is how api.weather.gov answers a location outside its US coverage: the four venues that sit outside it would otherwise page an operator about NWS being down every time someone opened a Toronto page, and that is precisely how an alarm becomes noise (`isNwsOutageStatus`, pinned by a test). The convention is now a guardrail at the top of `CLAUDE.md`: **any upstream that degrades quietly registers in `data-source-health.ts` in the same change**, alert text says what it costs the customer, a degraded path gets its own row, a known permanent absence is not an outage, and nothing invents data to fill a gap.

- **2026-08-29**: **ZIP pages stopped showing simulated weather, and every upstream now reports its own silence.** Both per Derek, after the two outages found earlier the same evening. **(1) The public forecast.** A ZIP page now has three honest states: the live Wager on Weather consensus; the **last real consensus we hold** for that location (kept up to 6 hours) behind an amber banner naming its age, when the upstream is down; or, when we hold nothing real, a **notice in place of the forecast body** plus **HTTP 503 with `Retry-After`** — the correct answer for temporarily missing content, and one that tells a crawler to come back rather than index an empty page. The simulated fallback is never rendered. **(2) The alarm.** New `data-source-health.ts`: every fallback path now records whether its upstream answered, and three consecutive failures raises an alert naming the source and **what it costs the customer** ("live scores, quarter/half and game clock disappear from the Weatherboards..."), because an alert nobody can act on gets ignored. ESPN's canonical host is tracked separately from ESPN itself: it was blocked for hours while the mirror served every request, so the site looked perfect, and that is precisely the state worth seeing early. **It is the one source that records without alerting** — it has been blocked continuously since 2026-08-29 with the mirror covering it, so alerting would fire every ten minutes forever about something with no customer impact and no action to take. Read it as a row on the health page, not as a page. Visible on `/admin/system/health` under **Upstream Data Sources**. It is cheap by design: one Redis write per failure, and at most one per source every 30 seconds on the success path, all of it swallowing its own errors so health bookkeeping can never be why a page fails. Tests in `tests/data-source-health.test.ts` pin the thresholds (one failure is noise, three in a row is an outage, a never-reached source reads "no data yet" rather than green).

- **2026-08-29**: **The market engines could price on invented weather, and did.** Found while reading the pricing cron's logs: `Open-Meteo failed, falling back to mock data` inside **five separate `/api/cron/auto-hvl-pricing` runs in 45 minutes**. `tryOpenMeteoOrMock` has always answered an Open-Meteo failure with `getMockForecast` — a seeded pseudo-random generator (a latitude sine curve plus noise, `mock-data.ts`) — and returned it stamped `source: getForecastSource('open-meteo')`, the identical source object a real fetch returns. Nothing downstream could tell them apart, and `prefetchVenueForecasts` feeds exactly this to the four auto-market engines. **This is the 0.5F-market family one level nastier:** not a null coercing to a zero anyone would notice, but a plausible mid-range number with nothing behind it. **The 2026-08-28 bad-line alarm could not have caught it** — it corroborates a line against that venue's own hourly forecast, which on a simulated response is the same invented series, and a generator is internally consistent by construction. Five changes: **(1)** a generated forecast now carries `synthetic: true` and a source label that does not claim to be Open-Meteo; **(2)** it is never written to the shared forecast cache, so a momentary 429 no longer pins fabricated numbers in front of every reader of that venue for the full 10-minute TTL; **(3)** the engines refuse a simulated venue forecast, treating it exactly like a missing one (game skipped, retried next tick) and raising a **critical** `forecast_simulated` alert; **(4)** the line audit and the operator Suggest-Lines helper both refuse it; **(5)** the 429 itself is now attacked at the source, with one retry after a short backoff and a **ceiling of 6 concurrent venue fetches** in `prefetchVenueForecasts`, which had been an unbounded `Promise.all` over every venue in a league (about 30 for MLB) across four engines on staggered crons. Tests in `tests/forecast-integrity.test.ts`, including one that demonstrates the audit passing with full confidence on the simulated numbers, so nobody removes the provenance flag believing the audit already covers this.
