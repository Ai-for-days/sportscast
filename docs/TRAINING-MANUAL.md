...
**Every upstream reports its own silence (2026-08-29)** — new `data-source-health.ts` tracks ESPN (and, separately, ESPN's canonical host, which is the early warning), Open-Meteo, NWS and The Odds API. Three consecutive failures raises an alert naming the source AND what it costs the customer; a success clears the streak. Read it on `/admin/system/health` under **Upstream Data Sources**. **Why it exists:** on this date, two upstreams were dark for hours and the site looked completely normal, because every one of them has a fallback and a working fallback is exactly what makes an outage invisible. Logs are not an alarm. If you are adding a new upstream with a fallback, add it here in the same change.
