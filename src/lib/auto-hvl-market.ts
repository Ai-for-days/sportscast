...
async function processGame(league: SiteLeague, g: EnrichedScheduleGame, budget: CreationBudget, forecasts: VenueForecastMap): Promise<AutoHvLOutcome> {
  ...
  try {
    if (existingId) {
      ...
      // extreme weather.
      for (const [value, fc, label] of [[homeHigh, homeForecast, 'home high'], [homeLow, homeForecast, 'home low'], [awayHigh, awayForecast, 'away high'], [awayLow, awayForecast, 'away low']] as [number, typeof homeForecast, string][]) {
      ...
      if (existingPs.spread === spread && !lockNeedsFix) return { ...base, action: 'unchanged', wagerId: existing.id };
      await updateWager(existing.id, lockNeedsFix ? { spread, lockTime: lockTimeIso } : { spread });
