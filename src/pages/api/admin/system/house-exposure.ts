...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (action === 'summary') {
      const [summary, snapshots] = await Promise.all([getExposureSummary(), listSnapshots(60)]);
      return jsonResponse({ summary, snapshots });
    }
    ...
    return jsonResponse({ error: 'house_exposure_failed', message: err?.message ?? String(err) }, 500);
  }
};
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (action === 'generate-snapshot') {
      const scope = (body.scope ?? 'all') as ExposureScope;
    ...
    if (err instanceof HouseExposureError) {
      return jsonResponse({ error: err.code, message: err.message }, 400);
    }
    return jsonResponse({ error: 'house_exposure_action_failed', message: err?.message ?? String(err) }, 500);
  }
};
