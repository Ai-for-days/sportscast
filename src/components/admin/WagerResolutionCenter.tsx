          ...
          <tbody>
            {wagers.map((w: any) => (
              <tr key={w.id} style={{ borderLeft: w.pastLockTime ? '3px solid #ef4444' : undefined }}>
                ...
                <td style={{ ...td, fontSize: 11, color: w.pastLockTime ? '#ef4444' : '#94a3b8' }}>{formatEasternDMYTime(w.lockTime)}</td>
                <td style={td}>
    ...
    <>
      <div style={card}>
        ...
        <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8 }}>
          ...
          <Field label="Lock time" value={formatEasternDMYTime(w.lockTime)} />
